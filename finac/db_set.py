...
currency_rate = Table('currency_rate', meta,
                      Column('currency_from_id', Integer,
                             ForeignKey('currency.id', ondelete='CASCADE'), nullable=True,
                             primary_key=True),
                      Column('currency_to_id', Integer,
                             ForeignKey('currency.id', ondelete='CASCADE'), nullable=False,
                             primary_key=True),
...
account = Table('account', meta,
                ...
                Column('currency_id', Integer, ForeignKey('currency.id', ondelete='CASCADE'),
                       nullable=False),
                Column('max_overdraft', Float),
                Column('max_balance', Float))

transact = Table('transact', meta,
                 Column('id', Integer, primary_key=True, autoincrement=True),
                 Column('account_credit_id', Integer, ForeignKey('account.id', ondelete='SET NULL')),
                 Column('account_debit_id', Integer, ForeignKey('account.id', ondelete='SET NULL')),
...
asset = Table('asset', meta,
              ...
              Column('currency_id', Integer, ForeignKey('currency.id', ondelete='CASCADE'),
                     nullable=False),
              Column('asset_group_id', Integer, ForeignKey('asset_group.id', ondelete='CASCADE'),
                     nullable=False),
              Column('amount', Float, nullable=False, default=0))

asset_price = Table('asset_price', meta,
                    Column('id', Integer, primary_key=True,autoincrement=True),
...
def init_db(engine):
    ...
    insert into currency(code) values(:code)"""), code=cur)
            ...
            logger.info('Currency successfully created')
